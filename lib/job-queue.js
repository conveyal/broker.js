...
export default class JobQueue {
  ...
  getJobs(graphIds: Array<String> = [], max: number = 10): {
    ...
    if (this.jobs.size === 0) {
      console.log('returning no jobs as there are none')
      return {
        jobId: null,
        tasks: []
      }
    ...
    if (nJobs === 0) {
      console.log('returning no jobs as all have been completed')
      return {
        jobId: null,
        tasks: []
      }
    ...
    console.log(`total weight after adjustment ${totalWeight}`)
    ...
    for (let [jobId, weight] of weightsByJob.entries()) {
      console.log(`job ${jobId} has weight ${weight}`)
